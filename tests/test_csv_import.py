...
실제 영웅문 CSV 샘플을 받으면 그 형식의 테스트를 추가해 고정한다.
"""
...
def test_영웅문_실물_형식_헤더와_따옴표_접두(tmp_path):
    content = (
        ...
        '신,,SK이노베이션,"120,300","2.82",119000 126700 116500 120300,"353,501",,\'096770\n'
...
def test_A접두_코드와_헤더행_및_중복_처리(tmp_path):
    ...
    assert parse_watchlist_csv(path) == [("005930", "삼성전자")]
...
def test_종목명이_없으면_코드로_대체(tmp_path):
    ...
    assert parse_watchlist_csv(path) == [("005930", "005930")]
