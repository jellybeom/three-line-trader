...
# 기준 설정: 1선 10,000 / 2선 9,000 / 3선 8,000, 1·2차 각 100주
...
def test_극소량_보유시_매도수량_0이면_주문없이_상태만_전이():
    p = Params(line1=10_000, line2=9_000, line3=8_000, buy1_qty=2, buy2_qty=2)
...
def test_대기에서_2선_이하_갭이면_1차2차_동시매수():
    ...
    assert pos.state is State.BUY2
    assert pos.total_bought == pos.remaining == 200  # 100 + 100 을 한 주문으로
...
def test_동시매수_후_익절은_체결_평단_기준():
    ...
    pos = run(pos, [8_755])
    assert pos.state is State.BUY2_TP1 and pos.remaining == 120
...
def test_관리자_리셋은_종료에서만_가능():
    ...
    assert reset(pos) == Position()  # 종료 → 대기, 완전 초기화
    holding = Position(
...
def test_애매한_수량도_익절_합계가_전량과_일치():
    p = Params(line1=10_000, line2=9_000, line3=8_000, buy1_qty=7, buy2_qty=7)
...
def test_잘못된_설정은_생성_시점에_실패():
    with pytest.raises(ValueError):
        Params(line1=9_000, line2=10_000, line3=8_000, buy1_qty=100, buy2_qty=100)
    with pytest.raises(ValueError):
        Params(
            line1=10_000,
            line2=9_000,
