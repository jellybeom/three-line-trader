...
P = Params(line1=10_000, line2=9_000, line3=8_000, buy1_qty=100, buy2_qty=100)
...
def test_등록_후_복원하면_설정과_포지션이_그대로(store, tmp_path):
    store.register_symbol("005930", "삼성전자", P)
    store = reopen(store, tmp_path)
    name, params, pos = store.load_all()["005930"]
    assert name == "삼성전자"
...
def test_종목코드_앞자리_0이_보존됨(store):
    store.register_symbol("005930", "삼성전자", P)
    assert "005930" in store.load_all()  # INTEGER 였다면 5930 이 됐을 것
...
def test_오버나이트_상태로_등록하면_그_상태로_복원(store, tmp_path):
    ...
    store.register_symbol("005930", "삼성전자", P, overnight)
    store = reopen(store, tmp_path)
    _, _, pos = store.load_all()["005930"]
...
def test_재등록하면_설정이_대체되고_이전_상태가_이벤트에_남음(store):
    store.register_symbol("005930", "삼성전자", P)
    p2 = Params(line1=11_000, line2=10_000, line3=9_000, buy1_qty=50, buy2_qty=50)
    store.register_symbol("005930", "삼성전자", p2)
    _, params, _ = store.load_all()["005930"]
    assert params == p2
...
def test_전이_저장은_포지션과_이벤트를_함께_남김(store):
    ...
    store.save_transition("005930", State.WAITING, pos, d, price=9_950)

    _, _, restored = store.load_all()["005930"]
...
def test_전체_사이클_후_재시작해도_최종_상태_복원(store, tmp_path):
    ...
    for price in [10_000, 10_300, 10_500, 10_700]:  # 매수 → 3% → 5% → 7% 청산
        ...
        store.save_transition("005930", state_before, pos, d, price)
...
def test_주문_전송_직후_죽어도_pending이_복원됨(store, tmp_path):
    ...
    store.save_position("005930", pos)
...
def test_관리자_리셋은_대기로_되돌리고_이벤트를_남김(store):
    ...
    store.register_symbol("005930", "삼성전자", P, closed)
    new_pos = store.admin_reset("005930", closed)
...
def test_관리자_리셋은_종료가_아니면_거부되고_DB도_불변(store):
    ...
    store.register_symbol("005930", "삼성전자", P, holding)
    with pytest.raises(ValueError):
        store.admin_reset("005930", holding)
    _, _, pos = store.load_all()["005930"]
...
def test_일반_로그와_기간_필터_조회(store):
    store.register_symbol("005930", "삼성전자", P)
    store.log("005930", "에러", "WebSocket 재연결")
