...
| 익절 트리거 %   | 평단가 대비 1 / 2 / 3차 익절 기준             | 3% / 5% / 7%     |
| 익절 비중       | **최초 보유 물량 기준** 매도 비율 (합계 100%) | 40% / 50% / 10%  |
| 매수 배분       | 1차 / 2차 매수 투입 수량·금액                 | 프로그램 내 설정 |
...
| 관심종목 설정  | **매일 저녁 사용자가 직접** 다음날 관심종목과 3선 가격을 등록·갱신. 신규 등록 종목은 '대기'로 시작하되, **시작 상태를 직접 지정할 수도 있음** (오버나이트 보유분은 전일 마감 상태로 시작). 상태와 보유 정보(평단·잔량)가 모순되는 입력은 등록 시점에 거부 |
...
| 본절 버퍼      | 수수료·거래세 고려한 버퍼(예: 평단 +0.3~0.5%)는 파라미터 자리만 확보, 기본 0 (추후 검토)                                                                                                                                                                  |
...
| 테이블      | 용도                                                                               |
| ----------- | ---------------------------------------------------------------------------------- |
| `symbols`   | 관심종목 및 종목별 설정 (3선 가격, 익절 %, 비중, 매수 배분)                        |
| `positions` | 종목별 현재 상태 스냅샷 (상태, 평단가, 최초 물량, 잔량, 진입일) — 장애 복구의 기준 |
| `events`    | 상태 전이·주문·체결 이벤트 이력 (append-only) — 월간 통계·시각화의 원천            |
...
├── config.toml               # API 키, Discord 토큰, 기본 파라미터
├── main.py                   # 진입점: 코어 스레드 기동 → Tkinter 앱 실행 (조립만 담당)
├── simulate.py               # 개발용 시뮬레이터: 가짜 틱으로 상태머신+저장소+UI 전체 구동
├── trader/
│   ├── __init__.py
│   ├── params.py             # 설정 로드, 종목별 파라미터
│   ├── state_machine.py      # 상태 정의 + 전이 로직 (순수 함수, I/O 없음)
│   ├── watcher.py            # 키움 WebSocket 시세 수신 / 재연결
│   ├── broker.py             # 키움 REST 주문 / 체결통보 / 토큰 관리
│   ├── store.py              # SQLite 영속화 / 복원
│   ├── notifier.py           # Discord 알림
│   ├── logger.py             # 로깅 설정
│   └── ui/                   # Tkinter UI (화면 단위 분리, 비즈니스 로직 없음)
│       ├── __init__.py
│       ├── app.py            # 메인 윈도우: 툴바(시작/일시정지·등록) + 분할 화면, 200ms 큐 폴링
│       ├── positions_view.py # 종목별 상태·평단·잔량·수익률 모니터 (우클릭: 리셋/제외)
│       ├── events_view.py    # 이벤트 로그 뷰 (최근 500줄)
│       ├── register_dialog.py# 종목 등록 별도 창 (시작 상태 지정 포함)
│       └── bus.py            # 코어 ↔ UI 큐 정의 + 메시지 타입 (프로토콜 문서 역할)
├── tests/
│   ├── test_state_machine.py # 상태 머신 단위 테스트
│   └── test_store.py         # 저장/복원, 장애 복구 테스트
└── data/
...
- [x] SQLite 저장/복원
- [ ] 키움 WebSocket 시세 수신
- [ ] 키움 REST 주문 실행 (모의투자 검증 → 실전)
- [ ] Discord 알림
- [x] Tkinter UI (관심종목 편집 / 상태 모니터 / 이벤트 로그, 큐 기반 코어 연동) + 개발용 시뮬레이터
- [ ] 관리자 개입 기능 (종료 → 대기 수동 전환)
- [ ] 본절 버퍼 옵션
- [ ] 종료 시 복기 차트 자동 생성 → Discord 전송 (분봉 REST 조회 + mplfinance, 3선·평단·체결 마커 표기, 별도 비동기 태스크로 처리)
- [ ] 월간 통계 시각화 (events 기반)
