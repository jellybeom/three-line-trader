...
| 1차 / 2차 매수 금액 | 전역 자금 설정에서 자동 채움, 종목별 수정 가능 | 종목당 배분의 50% / 50% |
...
├── main.py                   # 실전 진입점: watcher+broker+UI 조립 (broker 완성 후 작성)
...
│   ├── broker.py             # 키움 REST 주문 / 체결통보 / 계좌 조회 (다음 구현 단위)
│   ├── store.py              # SQLite 영속화 / 복원 (매매일 단위)
│   ├── notifier.py           # Discord 알림 (예정)
│   └── ui/                   # Tkinter UI (화면 단위 분리, 비즈니스 로직 없음)
│       ├── __init__.py
│       ├── app.py            # 메인 윈도우(FHD): 상시 툴바 + 접이식 연결/자금 바 + 손익 요약 + 상태 바
│       ├── positions_view.py # 상태·현재가·평단·잔량·수익률·실현손익·1/2/3선 모니터 (더블클릭 편집)
│       ├── events_view.py    # 이벤트 로그 뷰 (최근 500줄, 지우기/CSV 내보내기)
│       ├── register_dialog.py# 종목 등록/편집 별도 창 (시작 상태 지정, 편집 시 포지션 유지)
...
uv run check_kiwoom.py     # 키움 연결 점검 (config.toml 필요, 장중이 아니면 틱 0건이 정상)
uv run main.py             # 실전 모드 (broker 완성 후)
